...
var translations = map[string]translation{
	...
	"accounts.selectCurrency":                   {"Select a currency", "请选择币种", "請選擇幣種"},
	...
	"history.kind.fx_conversion":                {"Currency conversion", "账户内换汇", "帳戶內換匯"},
	...
	"history.soldCurrency":                      {"Sold currency", "卖出币种", "賣出幣種"},
	"history.boughtCurrency":                    {"Bought currency", "买入币种", "買入幣種"},
	"history.sentCurrency":                      {"Sent currency", "转出币种", "轉出幣種"},
	"history.receivedCurrency":                  {"Received currency", "转入币种", "轉入幣種"},
	"history.fxFee":                             {"Conversion fee", "换汇费用", "換匯費用"},
	...
	"history.selectAction":                      {"Select an action", "请选择操作", "請選擇操作"},
	"history.selectHolding":                     {"Select a Holding", "请选择持仓", "請選擇持倉"},
	"history.selectInstrument":                  {"Select an Instrument", "请选择标的", "請選擇標的"},
	...
	"nav.marketData":                            {"Market data", "市场数据", "市場資料"},
	...
	"onboarding.description":                    {"Create one local balance sheet and add the people whose ownership you want to track.", "创建一个本地资产负债表，并添加需要参与所有权分配的成员。", "建立一個本機資產負債表，並新增需要參與所有權分配的成員。"},
	"onboarding.householdName":                  {"Household name", "家庭名称", "家庭名稱"},
	"onboarding.householdNamePlaceholder":       {"Wang Household", "王氏家庭", "王氏家庭"},
	"onboarding.memberHint":                     {"Add at least one member. You can add more later from Members.", "至少添加一名成员；创建后仍可在“成员”页面继续添加。", "至少新增一名成員；建立後仍可在「成員」頁面繼續新增。"},
	"onboarding.memberNamePlaceholder":          {"Member name", "成员姓名", "成員姓名"},
	"onboarding.addMember":                      {"Add member", "添加成员", "新增成員"},
	"onboarding.removeMember":                   {"Remove", "删除", "刪除"},
	"onboarding.memberCount":                    {"%d members added", "已添加 %d 名成员", "已新增 %d 名成員"},
	...
	"page.marketDataDescription":                {"Manage reusable instruments, quotes, and FX preferences in one local-first workspace.", "在一个本地优先的工作区管理投资标的、报价和汇率偏好。", "在一個本機優先的工作區管理投資標的、報價和匯率偏好。"},
	"page.marketDataTitle":                      {"Market data", "市场数据", "市場資料"},
	"marketData.instruments":                    {"Instruments", "投资标的", "投資標的"},
	"marketData.fx":                             {"FX rates", "汇率", "匯率"},
	"marketData.fxLocalFirst":                   {"Quotes are stored locally and refreshed only by explicit user action.", "报价保存在本地，仅在用户明确操作时刷新。", "報價儲存在本機，僅在使用者明確操作時重新整理。"},
	...
	"portfolio.useCurrentPrice":                 {"Use current price", "使用当前价格", "使用目前價格"},
