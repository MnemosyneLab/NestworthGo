...
func (s *Service) ArchiveInstrument(ctx context.Context, id domain.InstrumentID, archived bool) error {
	...
	s.changeMu.Lock()
	defer s.changeMu.Unlock()
	return safePortfolioError(s.repository.SetInstrumentArchive(ctx, household.ID, id, archived, s.clock()))
}
...
func (s *Service) SetInstrumentLogo(ctx context.Context, id domain.InstrumentID, assetID domain.MediaAssetID) error {
	...
	return safePortfolioError(s.repository.SetInstrumentLogo(ctx, household.ID, id, assetID, s.clock()))
}
...
func (s *Service) SetInstrumentQuoteSource(ctx context.Context, id domain.InstrumentID, source string) error {
	...
	if origin, originErr := s.repository.HistoryOrigin(ctx, household.ID); originErr != nil {
		...
		return safePortfolioError(s.repository.SetInstrumentQuoteSourceWithObservation(ctx, household.ID, id, parsed, observation))
	}
	return safePortfolioError(s.repository.SetInstrumentQuoteSource(ctx, household.ID, id, parsed, s.clock()))
}
