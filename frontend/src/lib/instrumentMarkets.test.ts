...
const CATALOG = ["ASX", "SSE", "SZSE", "NASDAQ", "NYSE", "AMEX", "ARCA", "BZX", "EDGX", "IEX", "HKEX", "SGX"];

describe("instrument market country mapping", () => {
  it("maps a market to its country", () => {
    expect(countryForMarket("NASDAQ")).toBe("US");
    expect(countryForMarket("ARCA")).toBe("US");
    expect(countryForMarket("BZX")).toBe("US");
  ...
  it("filters markets to the selected country and keeps a saved outlier", () => {
    ...
    expect(marketsForCountry(CATALOG, "US")).toEqual(["NASDAQ", "NYSE", "AMEX", "ARCA", "BZX", "EDGX", "IEX"]);
    expect(marketsForCountry(CATALOG, "US", "SGX")).toEqual(["NASDAQ", "NYSE", "AMEX", "ARCA", "BZX", "EDGX", "IEX", "SGX"]);
  });
});
