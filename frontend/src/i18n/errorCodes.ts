/**
 * The `errorCode` i18next namespace, keyed directly by `domain.ErrorCode`
 * (`domain.ErrorCode` is the single source of truth for the i18next
 * `error.<code>` key namespace; a new domain error code and a new i18next key
 * must ship together).
 *
 * Error messages are keyed by stable backend error codes rather than English
 * prose. The Wails error helper resolves these codes and combines them with
 * localized field labels when a backend error identifies a specific input.
 *
 * Every key here MUST have a counterpart in every locale; missing-key
 * coverage is checked by the locale-coverage test.
 */
export type ErrorCodeCatalog = Record<string, string>;

export const errorCodesEn: ErrorCodeCatalog = {
  validation: "This value is not valid.",
  not_found: "This could not be found.",
  conflict: "This could not be completed because of a conflict.",
  unsupported_database: "This database format is not supported.",
  migration_failed: "The database could not be upgraded.",
  integrity_failed: "A data integrity check failed.",
  unavailable: "This is currently unavailable.",
  database_upgrade_required: "This database was created by an older Nestworth version. Keep the file and restore or import through a supported path.",
  database_from_newer_version: "This database was created by a newer Nestworth version. Open it with a compatible app.",
  database_integrity_failed: "The local database failed an integrity check. Keep the file and restore from a backup.",
  database_unavailable: "The local database could not be opened. Check permissions and disk space, then retry.",
  decimal_overflow: "This value is outside the supported range.",
  provider_unavailable: "The market data provider is currently unavailable.",
  provider_authentication: "The market data provider rejected authentication.",
  provider_rate_limit: "The market data provider's rate limit was reached.",
  unsupported_provider_symbol: "This provider symbol is not supported.",
  malformed_provider_response: "The provider returned an invalid response.",
  market_data_response_too_large: "The provider response was too large.",
  history_not_started: "Start history before recording a change.",
  history_timezone_required: "Confirm a Household timezone before continuing.",
  invalid_change: "This change is not valid.",
  invalid_change_time: "This change time is not valid.",
  no_change: "The new value is unchanged.",
  insufficient_balance: "There is not enough balance for this change.",
  insufficient_quantity: "There is not enough quantity for this change.",
  already_undone: "This change has already been undone.",
  cannot_fix_change: "This change cannot be fixed.",
  transfer_mismatch: "The transfer amounts do not match.",
  invalid_trade: "This trade is not valid.",
  history_update_failed: "The history could not be updated.",
  cost_basis_required: "A per-unit cost is required for this change.",
  backup_invalid_format: "This backup file is not a valid Nestworth backup.",
  backup_missing_member: "This backup file is missing a required part.",
  backup_checksum_failed: "This backup file failed its integrity check.",
  backup_schema_unsupported: "This backup was created with an unsupported database format.",
  backup_integrity_failed: "This backup could not be verified.",
  backup_business_invariant_failed: "This backup does not satisfy Nestworth's data rules.",
  backup_restore_confirmation_required: "Type RESTORE to confirm replacing the current database.",
  backup_restore_busy: "A backup, restore, or import is already in progress.",
  backup_restore_swap_failed: "The current database could not be replaced.",
  backup_restore_rollback_failed: "Restore could not be completed and the previous database could not be recovered.",
  csv_invalid_encoding: "This CSV file is not valid UTF-8.",
  csv_invalid_format: "This CSV file could not be read.",
  csv_mapping_required: "Map every required column before continuing.",
  csv_mapping_conflict: "A source column is mapped to more than one field.",
  csv_row_invalid: "This CSV row cannot be imported.",
  csv_reference_unresolved: "This CSV row refers to a name that could not be resolved.",
  csv_duplicate: "This row conflicts with existing data or another row in the file.",
  csv_limit_exceeded: "This CSV file exceeds the supported size or row limit.",
  csv_commit_failed: "The CSV import could not be written.",
  internal: "An unexpected error occurred.",
};

export const errorCodesZhCN: ErrorCodeCatalog = {
  validation: "该值无效。",
  not_found: "未找到。",
  conflict: "因存在冲突，操作未能完成。",
  unsupported_database: "不支持此数据库格式。",
  migration_failed: "数据库升级失败。",
  integrity_failed: "数据完整性检查失败。",
  unavailable: "当前不可用。",
  database_upgrade_required: "此数据库由较旧版本的 Nestworth 创建。请保留该文件，并通过支持的路径恢复或导入。",
  database_from_newer_version: "此数据库由较新版本的 Nestworth 创建。请使用兼容的应用打开。",
  database_integrity_failed: "本地数据库未通过完整性检查。请保留该文件，并从备份恢复。",
  database_unavailable: "无法打开本地数据库。请检查权限和磁盘空间后重试。",
  decimal_overflow: "该数值超出支持的范围。",
  provider_unavailable: "行情数据提供方当前不可用。",
  provider_authentication: "行情数据提供方拒绝了认证。",
  provider_rate_limit: "已达到行情数据提供方的速率限制。",
  unsupported_provider_symbol: "不支持此数据提供方标的。",
  malformed_provider_response: "数据提供方返回了无效的响应。",
  market_data_response_too_large: "数据提供方响应过大。",
  history_not_started: "请先开始历史记录，再记录变化。",
  history_timezone_required: "请先确认家庭时区，再继续。",
  invalid_change: "此变化无效。",
  invalid_change_time: "此变化时间无效。",
  no_change: "新值与原值相同，未发生变化。",
  insufficient_balance: "余额不足，无法完成此变化。",
  insufficient_quantity: "数量不足，无法完成此变化。",
  already_undone: "此变化已被撤销。",
  cannot_fix_change: "此变化无法被修正。",
  transfer_mismatch: "转账金额不匹配。",
  invalid_trade: "此交易无效。",
  history_update_failed: "历史记录更新失败。",
  cost_basis_required: "此变化需要填写单位成本。",
  backup_invalid_format: "此备份文件不是有效的 Nestworth 备份。",
  backup_missing_member: "此备份文件缺少必要内容。",
  backup_checksum_failed: "此备份文件未通过完整性校验。",
  backup_schema_unsupported: "此备份使用了不受支持的数据库格式。",
  backup_integrity_failed: "此备份无法通过验证。",
  backup_business_invariant_failed: "此备份不符合 Nestworth 的数据规则。",
  backup_restore_confirmation_required: "请输入 RESTORE 以确认替换当前数据库。",
  backup_restore_busy: "备份、恢复或导入正在进行中。",
  backup_restore_swap_failed: "无法替换当前数据库。",
  backup_restore_rollback_failed: "恢复未能完成，且无法还原恢复前的数据库。",
  csv_invalid_encoding: "此 CSV 文件不是有效的 UTF-8。",
  csv_invalid_format: "无法读取此 CSV 文件。",
  csv_mapping_required: "请先映射所有必填列，再继续。",
  csv_mapping_conflict: "一个源列被映射到了多个字段。",
  csv_row_invalid: "此 CSV 行无法导入。",
  csv_reference_unresolved: "此 CSV 行引用的名称无法解析。",
  csv_duplicate: "此行与现有数据或文件中的其他行冲突。",
  csv_limit_exceeded: "此 CSV 文件超出支持的大小或行数限制。",
  csv_commit_failed: "CSV 导入未能写入。",
  internal: "发生了意外错误。",
};

export const errorCodesZhTW: ErrorCodeCatalog = {
  validation: "此數值無效。",
  not_found: "找不到。",
  conflict: "因存在衝突，操作未能完成。",
  unsupported_database: "不支援此資料庫格式。",
  migration_failed: "資料庫升級失敗。",
  integrity_failed: "資料完整性檢查失敗。",
  unavailable: "目前無法使用。",
  database_upgrade_required: "此資料庫由較舊版本的 Nestworth 建立。請保留該檔案，並透過支援的路徑還原或匯入。",
  database_from_newer_version: "此資料庫由較新版本的 Nestworth 建立。請使用相容的應用程式開啟。",
  database_integrity_failed: "本機資料庫未通過完整性檢查。請保留該檔案，並從備份還原。",
  database_unavailable: "無法開啟本機資料庫。請檢查權限和磁碟空間後再試。",
  decimal_overflow: "此數值超出支援的範圍。",
  provider_unavailable: "行情資料提供方目前無法使用。",
  provider_authentication: "行情資料提供方拒絕了驗證。",
  provider_rate_limit: "已達到行情資料提供方的速率限制。",
  unsupported_provider_symbol: "不支援此資料提供方標的。",
  malformed_provider_response: "資料提供方回傳了無效的回應。",
  market_data_response_too_large: "資料提供方回應過大。",
  history_not_started: "請先開始歷史記錄，再記錄變化。",
  history_timezone_required: "請先確認家庭時區，再繼續。",
  invalid_change: "此變化無效。",
  invalid_change_time: "此變化時間無效。",
  no_change: "新值與原值相同，未發生變化。",
  insufficient_balance: "餘額不足，無法完成此變化。",
  insufficient_quantity: "數量不足，無法完成此變化。",
  already_undone: "此變化已被撤銷。",
  cannot_fix_change: "此變化無法被修正。",
  transfer_mismatch: "轉帳金額不符。",
  invalid_trade: "此交易無效。",
  history_update_failed: "歷史記錄更新失敗。",
  cost_basis_required: "此變化需要填寫單位成本。",
  backup_invalid_format: "此備份檔不是有效的 Nestworth 備份。",
  backup_missing_member: "此備份檔缺少必要內容。",
  backup_checksum_failed: "此備份檔未通過完整性檢查。",
  backup_schema_unsupported: "此備份使用了不支援的資料庫格式。",
  backup_integrity_failed: "此備份無法通過驗證。",
  backup_business_invariant_failed: "此備份不符合 Nestworth 的資料規則。",
  backup_restore_confirmation_required: "請輸入 RESTORE 以確認取代目前資料庫。",
  backup_restore_busy: "備份、還原或匯入正在進行中。",
  backup_restore_swap_failed: "無法取代目前資料庫。",
  backup_restore_rollback_failed: "還原未能完成，且無法復原還原前的資料庫。",
  csv_invalid_encoding: "此 CSV 檔不是有效的 UTF-8。",
  csv_invalid_format: "無法讀取此 CSV 檔。",
  csv_mapping_required: "請先對應所有必填欄，再繼續。",
  csv_mapping_conflict: "一個來源欄被對應到多個欄位。",
  csv_row_invalid: "此 CSV 列無法匯入。",
  csv_reference_unresolved: "此 CSV 列引用的名稱無法解析。",
  csv_duplicate: "此列與現有資料或檔案中的其他列衝突。",
  csv_limit_exceeded: "此 CSV 檔超出支援的大小或列數限制。",
  csv_commit_failed: "CSV 匯入未能寫入。",
  internal: "發生了意外的錯誤。",
};
