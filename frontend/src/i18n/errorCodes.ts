 ...
 * (technical design Sec5: "domain.ErrorCode ... is the single source of
 * truth for the i18next `error.<code>` key namespace; a new domain error
 * code and a new i18next key must ship together").
 *
 * This intentionally does NOT reuse the ported `error.*` namespace from
 * internal/i18n/errors.go (see i18n/locales/*.json, generated by
 * scripts/port-i18n-catalog): that namespace keyed messages by the exact
 * English prose Fyne matched against, which is the brittle pattern this
 * migration replaces. It stays available for one-off UI copy that isn't
 * error-shaped, but the error-unwrapping helper (lib/wails.ts) only reads
 * from this namespace, combined with the ported `error.field.*` labels
 * for field-specific context.
 *
 * Every key here MUST have a counterpart in every locale; missing-key
 * coverage is checked by a Phase 5 locale-coverage test
 * (implementation plan Phase 5's Required Tests).
 */
