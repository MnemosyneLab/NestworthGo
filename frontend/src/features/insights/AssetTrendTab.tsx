...
function metricLabel(t: (key: string) => string, value: string): string {
  const key: Record<string, string> = { net_worth: "metricNetWorth", assets: "metricAssets", liabilities: "metricLiabilities", income: "income", spending: "spending", fees: "fees", dividend_interest: "components.dividendInterest", external_flow: "externalFlows", price_change: "components.priceChange", fx_impact: "components.fxImpact", fx_conversion_spread: "components.fxConversionSpread", investment_return: "metricInvestmentReturn", return_rate: "metricReturnRate", net_change: "metricNetChange", residual: "components.residual" };
...
export function AssetTrendTab({ session }: { session: AnalysisSessionState }) {
  ...
  const toolbar = (
    <div className="flex flex-wrap items-end gap-3">
      ...
      <div className="flex min-w-52 flex-col gap-1.5"><label htmlFor="asset-trend-metric" className="text-sm font-medium">{t("insights.metric")}</label><NativeSelect id="asset-trend-metric" value={metric} onChange={(event) => setMetric(event.target.value)}><optgroup label={t("insights.metricAssetsGroup")}><option value="net_worth">{metricLabel(t, "net_worth")}</option><option value="assets">{metricLabel(t, "assets")}</option><option value="liabilities">{metricLabel(t, "liabilities")}</option></optgroup><optgroup label={t("insights.metricCashFlowGroup")}><option value="external_flow">{metricLabel(t, "external_flow")}</option><option value="income">{metricLabel(t, "income")}</option><option value="spending">{metricLabel(t, "spending")}</option><option value="fees">{metricLabel(t, "fees")}</option></optgroup><optgroup label={t("insights.metricPerformanceGroup")}><option value="investment_return">{metricLabel(t, "investment_return")}</option><option value="return_rate">{metricLabel(t, "return_rate")}</option><option value="dividend_interest">{metricLabel(t, "dividend_interest")}</option><option value="price_change">{metricLabel(t, "price_change")}</option><option value="fx_impact">{metricLabel(t, "fx_impact")}</option><option value="fx_conversion_spread">{metricLabel(t, "fx_conversion_spread")}</option><option value="net_change">{metricLabel(t, "net_change")}</option><option value="residual">{metricLabel(t, "residual")}</option></optgroup></NativeSelect></div>
    </div>
  );
  let results;
  if (trend.isLoading) results = <LoadingState label={t("insights.loading")} />;
    ...
    <>
      {data.valueChange && <Card><CardContent className="pt-5"><AssetValueSummary summary={data.valueChange} /></CardContent></Card>}
      <Card>
  ...
  return <div className="flex flex-col gap-4" data-testid="asset-trend">{!data?.available && toolbar}{results}</div>;
}
