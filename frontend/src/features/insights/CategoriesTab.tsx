...
function DetailContent({ data, total, onOpenHistory }: { data: CategoryDetailDTO; total?: SignedMoneyView | null; onOpenHistory?: (filters: HistoryNavigationFilters) => void }) {
  const { t } = useTranslation();
  return <div className="flex flex-col gap-5 overflow-y-auto"><div><p className="text-xs uppercase tracking-wide text-muted-foreground">{t("insights.total")}</p><p className="mt-1 text-2xl font-semibold">{amountText(total)}</p></div>{data.valuationForced && <Badge variant="warning">{t("insights.valuationForced")}</Badge>}{data.status === "partial" && <p className="rounded-md border border-warning/40 bg-warning/10 p-3 text-sm text-warning-foreground">{data.missingReason ?? t("insights.partial")}</p>}{data.children && data.children.length > 0 && <div className="flex flex-col gap-2"><h3 className="text-sm font-medium">{t("insights.categoryChildren")}</h3><ul className="flex flex-col gap-1 text-sm">{data.children.map((child) => <li key={child.key} className="flex justify-between gap-3"><span className="truncate">{child.label}</span><span className="shrink-0">{amountText(child.amount)}</span></li>)}</ul></div>}<div className="flex flex-col gap-2"><h3 className="text-sm font-medium">{t("insights.records")}</h3>{data.activityRefs && data.activityRefs.length > 0 ? <ul className="flex flex-col gap-2 text-sm">{data.activityRefs.map((ref) => <li key={`${ref.date}-${ref.activityId ?? ref.accountId ?? ref.instrumentId ?? ref.amount?.amount ?? "record"}`} className="rounded-md border border-border px-3 py-2"><div className="flex justify-between gap-3"><span>{ref.date}</span><span>{amountText(ref.amount)}</span></div>{onOpenHistory && <Button type="button" variant="link" size="sm" className="mt-1 h-auto px-0" onClick={() => onOpenHistory({ from: ref.date, to: ref.date, accountId: ref.accountId || undefined, instrumentId: ref.instrumentId || undefined })}>{t("insights.viewInHistory")}</Button>}</li>)}</ul> : <p className="text-sm text-muted-foreground">{t("insights.noRecords")}</p>}</div></div>;
}
