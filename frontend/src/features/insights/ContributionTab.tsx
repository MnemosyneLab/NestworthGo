...
function ContributionRow({ row, rows, label, showRate, onSelect }: { row: ContributionRowDTO; rows: ContributionRowDTO[]; label: string; showRate: boolean; onSelect: () => void }) {
  ...
  return <li><Button type="button" variant="ghost" className="h-auto w-full justify-between gap-4 rounded-md px-3 py-2 text-left" onClick={onSelect}><span className="flex min-w-0 flex-1 items-center gap-2"><span className="h-1.5 min-w-8 max-w-40 flex-1 overflow-hidden rounded-full bg-muted" aria-hidden="true"><span className="block h-full rounded-full bg-primary/60" style={{ width: `${contributionBarWidth(row, rows)}%` }} /></span><span className="min-w-0 truncate">{label}</span></span><span className="flex shrink-0 items-center gap-4"><span>{row.amount ? amountText(row.amount) : t("insights.unavailableAmount")}</span>{showRate && <span className="w-20 text-right text-muted-foreground">{rateText(row.rate)}{coverageMark(row.ratedDays, row.totalDays)}</span>}</span></Button></li>;
}
...
export function ContributionTab({ session, onOpenHistory }: { session: AnalysisSessionState; onOpenHistory?: (filters: HistoryNavigationFilters) => void }) {
  ...
  const item = useContributionItem(context.request, returnType, groupBy, selected?.key ?? "", Boolean(selected) && context.enabled);
  ...
  if (context.origin.isLoading || contribution.isLoading) return <LoadingState label={t("insights.loading")} />;
  if (context.origin.isError || contribution.isError) return <ErrorState title={t("insights.error")} description={t("ui.state.errorDescription")} onRetry={() => { void (context.origin.isError ? context.origin.refetch() : contribution.refetch()); }} retryLabel={t("common.retryAction")} />;
  ...
  if (!data?.available) return contributionEmpty(data, returnType, t);

  return <div className="flex flex-col gap-4" data-testid="contribution"><div className="flex flex-wrap items-end gap-3 rounded-xl border border-border bg-card/60 p-4"><div className="flex flex-col gap-1.5"><label htmlFor="contribution-return-type" className="text-sm font-medium">{t("insights.returnType")}</label><NativeSelect id="contribution-return-type" value={returnType} onChange={(event) => { const next = event.target.value; setReturnType(next); setSelected(null); if (next === "realized" && (groupBy === "currency" || groupBy === "asset_class")) setGroupBy("instrument"); if (next !== "total_return" && (ordering === "rate_desc" || ordering === "rate_asc")) setOrdering("amount_desc"); }}><option value="total_return">{t("insights.totalReturn")}</option><option value="realized">{t("insights.realizedGain")}</option><option value="dividend_interest">{t("insights.dividendInterestView")}</option></NativeSelect></div><div className="flex flex-col gap-1.5"><label htmlFor="contribution-group-by" className="text-sm font-medium">{t("insights.groupBy")}</label><NativeSelect id="contribution-group-by" value={groupBy} onChange={(event) => { setGroupBy(event.target.value); setSelected(null); }}><option value="instrument">{t("insights.byInstrument")}</option><option value="account">{t("insights.byAccount")}</option>{returnType !== "realized" && <option value="currency">{t("insights.byCurrency")}</option>}{returnType !== "realized" && <option value="asset_class">{t("insights.byAssetClass")}</option>}</NativeSelect></div><div className="flex flex-col gap-1.5"><label htmlFor="contribution-sort" className="text-sm font-medium">{t("insights.sort")}</label><NativeSelect id="contribution-sort" value={ordering} onChange={(event) => setOrdering(event.target.value)}><option value="amount_desc">{t("insights.sortAmountDesc")}</option><option value="amount_asc">{t("insights.sortAmountAsc")}</option>{showRate && <option value="rate_desc">{t("insights.sortRateDesc")}</option>}{showRate && <option value="rate_asc">{t("insights.sortRateAsc")}</option>}<option value="name_asc">{t("insights.sortNameAsc")}</option></NativeSelect></div><p className="max-w-md text-sm text-muted-foreground">{t("insights.contributionIndependent")}</p></div><Card><CardHeader className="flex-row items-center justify-between gap-3"><CardTitle>{returnTypeLabel(t, returnType)}</CardTitle><span className="text-sm text-muted-foreground">{data.ratedDays}/{data.totalDays}</span></CardHeader><CardContent><ul className="divide-y divide-border" aria-label={t("insights.contribution")}>{(data.rows ?? []).map((row) => <ContributionRow key={row.key} row={row} rows={data.rows ?? []} label={labelFor(row)} showRate={showRate} onSelect={() => setSelected(row)} />)}</ul>{(data.rows ?? []).length === 0 && <EmptyState title={t("insights.noContributionData")} description={t("insights.noContributionDataHint")} />}</CardContent></Card><Sheet open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null); }}><SheetContent side="right"><SheetHeader><SheetTitle>{selected ? labelFor(selected) : t("insights.contributionDetail")}</SheetTitle></SheetHeader>{item.isLoading ? <LoadingState label={t("insights.loading")} /> : item.isError ? <ErrorState title={t("insights.error")} description={t("ui.state.errorDescription")} onRetry={() => item.refetch()} retryLabel={t("common.retryAction")} /> : item.data ? <ItemDetail data={item.data} returnType={returnType} accountNames={accountNames} instrumentNames={instrumentNames} onOpenHistory={onOpenHistory} /> : <EmptyState title={t("insights.noContributionData")} />}</SheetContent></Sheet></div>;
}
