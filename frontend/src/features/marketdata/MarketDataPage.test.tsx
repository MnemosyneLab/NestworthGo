...
function renderPage(focus?: HealthFocus) {
  const queryClient = createTestQueryClient();
  return render(
    <QueryClientProvider client={queryClient}>
      <MarketDataPage focus={focus} />
    </QueryClientProvider>,
  );
}
...
describe("MarketDataPage", () => {
  ...
  it("does not offer targeted repair for unsupported coverage", async () => {
    renderPage({ currencyA: "USD", currencyB: "CNY", action: "none", executable: false });
    expect(await screen.findByText(/Automatic repair is unavailable/)).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Preview repair" })).not.toBeInTheDocument();
    expect(previewSync).not.toHaveBeenCalled();
  });
  it("disables confirmation when targeted preview has no executable work", async () => {
    previewSync.mockResolvedValue({ estimatedRequestCount: 0, snapshotWorkEstimate: 0, items: [], unresolved: [] });
    renderPage({ currencyA: "USD", currencyB: "CNY", action: "repair", executable: true });
    await userEvent.click(await screen.findByRole("button", { name: "Preview repair" }));
    expect(await screen.findByText(/No executable repair work/)).toBeInTheDocument();
    expect(within(screen.getByRole("dialog")).getByRole("button", { name: "Repair" })).toBeDisabled();
    expect(startSync).not.toHaveBeenCalled();
  });
  it("sends the focused manual FX date unchanged to the backend", async () => {
    appendManualFXQuote.mockResolvedValue({});
    renderPage({ currencyA: "USD", currencyB: "CNY", action: "manual_entry", rangeStart: "2026-09-01", rangeEnd: "2026-09-02" });
    await userEvent.click(await screen.findByRole("button", { name: /manual FX/i }));
    await userEvent.type(screen.getByLabelText("Rate"), "7");
    await userEvent.click(screen.getByRole("button", { name: /save manual FX/i }));
    await waitFor(() => expect(appendManualFXQuote).toHaveBeenCalledWith("USD", "CNY", "7", "2026-09-01"));
  });
