...
export function DirectoryPage() {
  ...
  const [archived, setArchived] = useState({ member: false, institution: false, group: false });
  const members = useMembers(archived.member); const createMember = useCreateMember(); const updateMember = useUpdateMember(); const archiveMember = useArchiveMember(); const setMemberIcon = useSetMemberIcon();
  const institutions = useInstitutions(archived.institution); const createInstitution = useCreateInstitution(); const updateInstitution = useUpdateInstitution(); const archiveInstitution = useArchiveInstitution(); const setInstitutionIcon = useSetInstitutionIcon();
  const groups = useGroups(archived.group); const createGroup = useCreateGroup(); const updateGroup = useUpdateGroup(); const archiveGroup = useArchiveGroup(); const setGroupIcon = useSetGroupIcon();
  ...
  return <div className="flex flex-col gap-6"><PageChrome pageId="directory" title={t("nav.directory")} /><PageIntro description={t("directory.description")} />
    <Tabs defaultValue="members"><TabsList><TabsTrigger value="members">{t("nav.members")}</TabsTrigger><TabsTrigger value="institutions">{t("nav.institutions")}</TabsTrigger><TabsTrigger value="groups">{t("nav.groups")}</TabsTrigger></TabsList>
      <TabsContent value="members"><DirectoryEntityList kind="member" showArchived={archived.member} onShowArchived={value => setArchived(previous => ({ ...previous, member: value }))} entities={members.data ?? undefined} isLoading={members.isLoading} isError={members.isError}
      ...
      <TabsContent value="institutions"><DirectoryEntityList kind="institution" showArchived={archived.institution} onShowArchived={value => setArchived(previous => ({ ...previous, institution: value }))} entities={institutions.data ?? undefined} isLoading={institutions.isLoading} isError={institutions.isError}
      ...
      <TabsContent value="groups"><DirectoryEntityList kind="group" showArchived={archived.group} onShowArchived={value => setArchived(previous => ({ ...previous, group: value }))} entities={groups.data ?? undefined} isLoading={groups.isLoading} isError={groups.isError}
