...
export function ReservationManager({ source, sources = [], open, onOpenChange }: { sources?: LiquiditySourceDTO[]; source: LiquiditySourceDTO | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  if (editing) return <ReservationSheet key={editing === "new" ? "new" : `${editing.id}:${editing.revision}`} source={source} reservation={editing === "new" ? undefined : editing} open={open} onOpenChange={(next) => { if (!next) setEditing(null); }} />;
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent className="overflow-y-auto">
    ...
    {source && <><p>{source.displayName}</p><Button onClick={() => setEditing("new")}>{t("availableFunds.addReservation")}</Button></>}
    ...
    {items.map((r) => <div key={r.id} className="my-3 flex flex-col gap-2 rounded-md border p-3">
      <p className="text-muted-foreground">{source?.displayName ?? sources.find((s) => key(s.sourceRef) === key(r.sourceRef))?.displayName ?? t("availableFunds.reservationSourceUnavailable")}</p>
