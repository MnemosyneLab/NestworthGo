...
import { useCreateHolding, useCreateInstrument, useInstruments } from "@/queries/investments";
      ...
      <SheetContent>
        ...
        <div className="overflow-y-auto">
          {waitingForHistory ? (
            ...
            <SimpleValueForm record={record} onDone={onRecorded} />
          ) : (
            <LockedChangeForm action={action} record={record} onDone={onRecorded} />
          )}
        </div>
      </SheetContent>
    </Sheet>
      ...
      {creatingInstrument ? (
        ...
        <>
          <div className="flex flex-col gap-1.5">
            ...
            <NativeSelect id="position-instrument" value={instrumentId} onChange={(event) => setInstrumentId(event.target.value)}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input id="position-unit-cost" inputMode="decimal" value={unitCost} onChange={(event) => setUnitCost(event.target.value)} />
          </div>
          {error && (
            <p role="alert" className="text-sm text-destructive">
              {error}
...
function SimpleValueForm({ record, onDone }: { record: AccountRecordDTO; onDone: () => void }) {
  const { t } = useTranslation();
  ...
  return (
    <div className="flex flex-col gap-4" role="form" aria-label={t("accounts.updateValue")}>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="simple-amount">{t("accounts.amount")}</Label>
        <Input id="simple-amount" inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} autoFocus />
      ...
      <Button type="button" onClick={submit} disabled={appendValue.isPending}>
...
function LockedChangeForm({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <RecordChangeForm key={kind} initial={initial} lock={lock} onRecorded={onDone} />
    </div>
  );
}
