...
Nestworth `0.3.4` is a Go 1.26 module with a Wails v3 desktop shell and a
...
```bash
GOCACHE=/tmp/nestworth-go-0.3.4 go test ./...
GOCACHE=/tmp/nestworth-go-0.3.4 go test -race ./...
GOCACHE=/tmp/nestworth-go-0.3.4 go vet ./...
...
`dist/macos/Nestworth-0.3.4-arm64.dmg`. Use isolated database and settings
...
manual. Wails v3 is pinned at `v3.0.0-beta.23` (`@wailsio/runtime` at
`3.0.0-beta.23`); upgrade those versions only in a dedicated change.
