...
**Green acceptance** (PLAN §3) additionally required DEF-R2-02 fixed → **met**. The remaining acceptance gate is native Wails replay for the corrected D5/D6 flows, the missing-price/missing-fx/zh-CN desktop matrix, and GUI-level D8 mutation evidence; automated replacements are recorded in `REMEDIATION-PLAN.md` and `probes/REMEDIATION-2026-09-09.json`.
...
- Native Wails replay on this macOS remediation host
...
| File | Case |
|------|------|
| `01-partial-banner.png`, `02-trend-all.png`, `02b-trend-all-retry.png`, `03-gap-day.png`, `04-drivers.png`, `RESULTS.json`, `RECHECK.json` | Existing missing-both shallow/recheck evidence; missing-price/missing-fx/zh-CN still pending |
...
2. Native replay the corrected D5/D6 flows and the missing-price/missing-fx/zh-CN matrix; keep `probes/REMEDIATION-2026-09-09.json` as automated evidence.
...
## Incomplete-matrix desktop (missing-both shallow)

| Check | Status | Notes |
|---|---|---|
| Partial banner | PASS | 21/45 |
| Trend All (DEF-R2-02) | **PASS** (recheck) | Full history loads; prior FAIL was YTD/All chip false positive |
| Gap day Aug 17 | PASS | Incomplete markers |
| Drivers incomplete | PASS | Partial warnings |
| missing-price / missing-fx desktop | SKIP | Probe matrix covers; desktop shallow = missing-both only |

Evidence: `screenshots/incomplete/` (+ `RECHECK.json`, `02b-trend-all-retry.png`).
